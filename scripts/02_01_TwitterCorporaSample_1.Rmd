...
save(data.twitter.sample, file = "./tmp/twitter_step2_sample.1.Rdata")
...
save(data.twitter.sample, file = "./tmp/twitter_step2_sample.2.Rdata")
rm(list = ls())
```
...
```{r libraries, include = T, echo = F, eval = F}
rm(list = ls())
...
lapply(twitter.corpora.transformed[100:110], as.character)
save(twitter.corpora.transformed, file = "./tmp/twitter.sample1.corpora.transformed.Rdata")

twitter.corpora.tdm.1g <- tdm.generate.ng(twitter.corpora.transformed)
twitter.corpora.tdm.1g
lapply(twitter.corpora.transformed[1:5], as.character)
save(twitter.corpora.tdm.1g, file = "./tmp/twitter.sample1.tdm.1g.Rdata")
...
lapply(twitter.corpora.transformed[1:5], as.character)
save(twitter.corpora.tdm.2g, file = "./tmp/twitter.sample1.tdm.2g.Rdata")
...
lapply(twitter.corpora.transformed[1:5], as.character)
save(twitter.corpora.tdm.3g, file = "./tmp/twitter.sample1.tdm.3g.Rdata")
...
##Twitter Corpora
load("./tmp/twitter.sample1.tdm.1g.Rdata")
...
save(corpora.allTermsFrequency, file = "./tmp/twitter.sample1.allTermsFrequency.1g.Rdata")
rm(twitter.corpora.tdm.1g)

load("./tmp/twitter.sample1.tdm.2g.Rdata")
corpora.allTermsFrequency <- NULL
twitter.corpora.tdm.2g
corpora.tdm <- twitter.corpora.tdm.2g
corpora.allTermsFrequency <- getAllTermsFrequencyInCorpora.as.df(corpora.tdm = corpora.tdm, chunck = 2000)
save(corpora.allTermsFrequency, file = "./tmp/twitter.sample1.allTermsFrequency.2g.Rdata")
rm(twitter.corpora.tdm.2g)

load("./tmp/twitter.sample1.tdm.3g.Rdata")
corpora.allTermsFrequency <- NULL
corpora.tdm <- twitter.corpora.tdm.3g
corpora.allTermsFrequency <- getAllTermsFrequencyInCorpora.as.df(corpora.tdm = corpora.tdm, chunck = 2000)
save(corpora.allTermsFrequency, file = "./tmp/twitter.sample1.allTermsFrequency.3g.Rdata")
rm(twitter.corpora.tdm.3g)
...
load("./tmp/twitter.sample1.allTermsFrequency.1g.Rdata")
twitter.allTerms.1g <- corpora.allTermsFrequency

load("./tmp/twitter.sample1.allTermsFrequency.2g.Rdata")
twitter.allTerms.2g <- corpora.allTermsFrequency

load("./tmp/twitter.sample1.allTermsFrequency.3g.Rdata")
...
```{r generateTermDocumentMatrixAndSummaryHeavyProcessing2, cache = T, eval = F}
rm(list = ls())
load(file = "./tmp/twitter_step2_sample.1.Rdata")
load(file = "./tmp/twitter.sample1.allTermsFrequency.1g.Rdata")

twitters.contains.in <- grepl("[[:space:]]in[[:space:]]", data.twitter.sample, ignore.case = F)
idx.in <- which(twitters.contains.in)
data.twitter.sample[idx.in]

twitters.contains.in_the <- grepl("[[:space:]]in the[[:space:]]", data.twitter.sample, ignore.case = F)
idx.in_the <- which(twitters.contains.in_the)
data.twitter.sample[idx.in_the]
...
# }
# 
# test <- "I like %$@to*&, chew;: gum, but don't like|}{[] bubble@#^)( gum!? in the"
# test.expected <- "I like    to    chew   gum  but don't like      bubble      gum   in the"
# result <- removePunctuations.exceptApostrophe(texts = test)
# test.expected == result
# result
# 
...
# }
# 
# tdm.generate.ng <- function(corpus){
#   TermDocumentMatrix(corpus) # create tdm from n-grams
# }
# 
require(tm)
y <- TermDocumentMatrix(Corpus(VectorSource("i am in the process of giving up")))
y
as.matrix(y)

require(tm)
y <- TermDocumentMatrix(Corpus(VectorSource("i am in the process of giving up")), control = list(wordLengths = c(1, Inf)))
y
as.matrix(y)
...
corpora.transformed <- corpora.transform("i am in the process of giving up")
lapply(corpora.transformed, as.character)

corpora.tdm.1g <- tdm.generate.ng(corpora.transformed)
corpora.tdm.1g
data.frame(as.matrix(corpora.tdm.1g))
findFreqTerms(corpora.tdm.1g,100)

x <- findFreqTerms(corpora.tdm.1g,0)
sum(x == "in")
```
