---
title: "Assignment: Milestone Report (Capstone Prject)"
...
* The amount of data available considering the tweets, news and blogs entries - such analysis is done using a representative sample can be used to infer facts about a population (considering limitations connected with the available processing hardware)
...
When loading the data using the following locale/ encoding has been used __`r Sys.getlocale("LC_CTYPE")` \ `r localeToCharset()`__. Inspecting the loaded data it is possible to identify some encoding issues (gremlings) due to the unrecognized characters (not supported languages, emoticons, etc)
...
### Entries (tweets, news and blogs) with a limited number of chars

#### Tweets

```{r tweetsShortDistribution}
hist(nchar(data.twitter.all), main = "No Of Chars Distribution - Tweets")
```

```{r tweetsShort, collapse=T}
...
```{r dataReductionForExploration, collapse=T, cache=T, eval=F}
set.seed(19711004)
