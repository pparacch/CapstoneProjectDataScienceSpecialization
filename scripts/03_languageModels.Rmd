...
t.counters.atLeast3 <- (t.counters >= 3)
t.terms <- t.terms[t.counters.atLeast3]
t.counters <- t.counters[t.counters.atLeast3]
t.N <- sum(t.counters)
t.V <- length(unique(t.terms))
...
save(t.terms, t.counters, t.N, t.V, data.bigrams, data.nextWord, data.prob, file = "./../data/processed/model.twitter.trigrams.Rdata")
format(object.size(data.bigrams), "auto")
format(object.size(data.nextWord), "auto")
format(object.size(data.prob), "auto")

load(file = "./../data/processed/model.twitter.trigrams.Rdata")

#Case 1
index <- which(data.bigrams == "a case")
data.nextWord[index]
data.prob[index]
